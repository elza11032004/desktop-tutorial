...
<?php
	...
	$sql = "SELECT * FROM `products2` WHERE `product_id`=".$id;
...
$sql = sprintf("SELECT * FROM `orders` WHERE `user_id`='%s' AND `product_id`='%s'",$_SESSION["user_id"], $id);
	...
	$connect->query(sprintf("UPDATE `products2` SET `count`='%s' WHERE `product_id`='%s'", --$product["count"], $product["product_id"]));

	return header("Location:../../cart2.php?message=Товар добавлен в корзину");
